...
SUPPORTED_PLATFORMS = {'gitlab', 'jenkins', 'circleci'}
...
class CICDGenerator:
    ...
    SUPPORTED_LANGUAGES = {
        'python': {
            ...
            'test_cmd': 'pytest tests/ --junitxml=test-results/results.xml',
    ...
    def ensure_output_dir(self, subdir: str = '') -> str:
    ...
    def generate_gitlab_ci(
        ...
        config = {
            ...
            'cache': {'paths': ['.cache/pip', 'venv/', 'node_modules/']},
        }
        ...
        for stage in stages:
            ...
            if stage == 'test':
                config[job_name]['before_script'] = ['mkdir -p test-results']
                config[job_name]['artifacts'] = {'reports': {'junit': 'test-results/results.xml'}}
...
"""
    ...
    def generate_all(self, platform: Optional[str] = None) -> None:
        """Generate CI/CD configs for all platforms or specific one"""
        if platform is not None and platform.lower() not in SUPPORTED_PLATFORMS:
            raise ValueError(
...
def main():
    ...
    parser.add_argument(
        '--language',
        choices=list(CICDGenerator.SUPPORTED_LANGUAGES.keys()),
        default='python',
