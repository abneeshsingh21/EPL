...
def cloud_configure(region=None, access_key=None, secret_key=None):
    ...
    Any parameter set to None is left unchanged.  After calling this
    function, all cached clients are invalidated so the next call
...
def cloud_s3_upload(bucket: str, key: str, file_path: str):
    ...
    Returns a map with bucket, key, and status.
    """
    s3 = _get_client('s3')
...
def cloud_s3_download(bucket: str, key: str, file_path: str):
    ...
    Returns a map with bucket, key, file_path, and status.
    """
    s3 = _get_client('s3')
...
def cloud_s3_list(bucket: str, prefix: str = ''):
    ...
    Returns a list of maps, each with key, size, and last_modified.
    """
    s3 = _get_client('s3')
...
def cloud_s3_create_bucket(bucket: str):
    ...
    Returns a map with bucket and status.
    """
    s3 = _get_client('s3')
...
def cloud_s3_list_buckets():
    ...
    Returns a list of maps with name and creation_date.
    """
    s3 = _get_client('s3')
...
def cloud_lambda_invoke(function_name: str, payload=None):
    ...
    payload should be a JSON-serializable dict/list or a string.
    Returns a map with status_code, payload (parsed), and function_error.
    """
...
def cloud_sqs_send(queue_url: str, message: str):
    ...
    Returns a map with message_id and status.
    """
    sqs = _get_client('sqs')
...
def cloud_sqs_receive(queue_url: str, max_messages: int = 1):
    ...
    Returns a list of maps with message_id, body, and receipt_handle.
    """
    sqs = _get_client('sqs')
