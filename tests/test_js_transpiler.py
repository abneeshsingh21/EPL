...
JS_CASES = [
    ...
    # The shim mirrors the interpreter's empty-input error (not a silent
    # `undefined`) for `max([])`/`min([])`.
    ('builtin_max_empty_guard', lambda: 'called on empty list' in to_js('Print max([])')),
