"""Tests for the epl-cloud package.

Tests are organized into three groups:
1. Module structure — verify the cloud domain is registered correctly
2. Backend unit tests — verify cloud_backend.py functions with mocked boto3
3. Stdlib dispatch — verify call_stdlib routes cloud_* names correctly
"""
...
class TestCloudBackend(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cloud_backend._boto3 = None  # Force re-import check

    def _mock_boto3(self):
        """Create a mock boto3 module and inject it into cloud_backend."""
        from epl import cloud_backend
    ...
    def test_s3_list_buckets(self):
        ...
        mock_s3.list_buckets.return_value = {
            'Buckets': [
                ...
                {'Name': 'bucket-2', 'CreationDate': datetime(2024, 6, 15)},
            ]
        }
        result = cloud_backend.cloud_s3_list_buckets()
...
class TestCloudStdlibDispatch(unittest.TestCase):
    ...
    def test_cloud_configure_dispatches(self):
        """cloud_configure should reach the backend without error."""
        from epl.stdlib import call_stdlib
        # Just testing the dispatch — configure with defaults
...
class TestCloudPyprojectIntegration(unittest.TestCase):
    """Verify pyproject.toml has the cloud dependency."""
