...
class TestBug02_AsyncServerHost:
    ...
    def test_async_server_stores_host(self):
        ...
        assert 'self.host' in source, 'AsyncEPLServer.__init__ does not store self.host'
    ...
    def test_async_server_uses_self_host_in_start_server(self):
        ...
        for match in matches:
            assert "'0.0.0.0'" not in match, f"AsyncEPLServer still hardcodes '0.0.0.0': {match}"
            assert 'self.host' in match, f'asyncio.start_server does not use self.host: {match}'
...
class TestBug03_RuntimeCEncoding:
    ...
    def test_no_default_encoding_reads_of_runtime_c(self):
        ...
        bad_pattern = re.findall(r"open\(runtime_path,\s*'r'\)\s*as", content)
...
class TestBug04_MainModule:
    ...
    def test_main_py_exists(self):
        ...
        assert os.path.exists(main_path), 'main.py does not exist — BUG-04 NOT FIXED'

    def test_main_exports_compile_file(self):
        """main.py must export compile_file."""
        import main

        assert hasattr(main, 'compile_file'), "main.py does not export 'compile_file'"

    def test_main_exports_cross_targets(self):
        """main.py must export CROSS_TARGETS."""
        import main

        assert hasattr(main, 'CROSS_TARGETS'), "main.py does not export 'CROSS_TARGETS'"

    def test_main_exports_interpreter(self):
        """main.py must export Interpreter."""
        import main

        assert hasattr(main, 'Interpreter'), "main.py does not export 'Interpreter'"

    def test_main_exports_lexer(self):
        """main.py must export Lexer."""
        import main

        assert hasattr(main, 'Lexer'), "main.py does not export 'Lexer'"

    def test_main_exports_parser(self):
        """main.py must export Parser."""
        import main

        assert hasattr(main, 'Parser'), "main.py does not export 'Parser'"
    ...
    def test_main_not_in_gitignore(self):
        ...
        with open(gitignore_path, 'r', encoding='utf-8') as f:
            content = f.read()
        assert '/main.py' not in content, 'main.py is still listed in .gitignore'
...
class TestBug05_FlaskImportOrder:
    ...
    def test_web_json_validates_args_before_flask(self):
        ...
        match = re.search(r"if name == 'web_json':(.*?)(?=\n    if name == )", src, re.DOTALL)
    ...
    def test_web_html_validates_args_before_flask(self):
        ...
        src = self._get_stdlib_source()
        match = re.search(r"if name == 'web_html':(.*?)(?=\n    if name == )", src, re.DOTALL)
    ...
    def test_web_redirect_validates_args_before_flask(self):
        ...
        src = self._get_stdlib_source()
        match = re.search(r"if name == 'web_redirect':(.*?)(?=\n    if name == )", src, re.DOTALL)
...
class TestBug07_ActiveConnectionsLock:
    ...
    def test_async_server_has_lock(self):
        ...
        source = inspect.getsource(AsyncEPLServer.__init__)
        assert 'asyncio.Lock()' in source, 'AsyncEPLServer.__init__ does not create asyncio.Lock()'
    ...
    def test_handle_connection_locks_increment(self):
        ...
        assert lock_pos < incr_pos, 'Lock is not acquired before incrementing _active_connections'
...
class TestBug08_ServerTimeout:
    ...
    def test_wait_for_server_timeout_is_adequate(self):
        ...
        assert timeout >= 20.0, f'Server timeout is {timeout}s — must be >= 20s for reliability'
    ...
    def test_health_check_timeout_is_adequate(self):
        ...
        timeout = float(match.group(1))
        assert timeout >= 1.0, f'Health check timeout is {timeout}s — must be >= 1s'
    ...
    def test_poll_interval_not_too_aggressive(self):
        ...
        assert interval >= 0.2, f'Poll interval is {interval}s — must be >= 0.2s to avoid CPU churn'
...
class TestBug09_UtcNowDeprecated:
    ...
    def test_stdlib_no_utcnow_call(self):
        ...
        match = re.search(
            r"if name == 'utc_now':(.*?)(?=\n        if name == )", content, re.DOTALL
        )
        assert match, 'Could not find utc_now handler'
        block = match.group(1)
        assert '.utcnow()' not in block, 'utc_now still uses deprecated datetime.utcnow()'
...
class TestBug10_UtcFromTimestampDeprecated:
    ...
    def test_stdlib_uses_timezone_aware_fromtimestamp(self):
        ...
        block = match.group(1)
        assert 'timezone.utc' in block, 'from_timestamp does not use timezone.utc'
...
class TestBug11_SilentExceptBlocks:
    ...
    def test_no_critical_uninstrumented_blocks(self):
        ...
        assert count <= 5, f'Found {count} uninstrumented silent except blocks — expected <= 5'
...
class TestBug12_ETagHash:
    ...
    def test_etag_uses_sha256(self):
        ...
        for line in etag_lines:
            assert 'md5' not in line.lower(), f'ETag still uses MD5: {line}'
            assert 'sha256' in line.lower(), f'ETag does not use SHA-256: {line}'
    ...
    def test_etag_format_is_valid(self):
        ...
        assert etag.startswith('"') and etag.endswith('"'), f'ETag not properly quoted: {etag}'
...
class TestIntegration:
    ...
    def test_no_hardcoded_0000_anywhere_in_web_server(self):
        ...
        bind_calls = re.findall(r"(?:ServerClass|start_server)\([^)]*'0\.0\.0\.0'[^)]*\)", content)
        assert len(bind_calls) == 0, f"Found hardcoded '0.0.0.0' in server bind calls: {bind_calls}"

    def test_all_deprecated_datetime_calls_removed(self):
        """No deprecated datetime calls should remain in stdlib.py."""
        with open(os.path.join(ROOT, 'epl', 'stdlib.py'), 'r', encoding='utf-8') as f:
            content = f.read()
        assert '.utcnow()' not in content, 'Deprecated datetime.utcnow() still exists in stdlib.py'
